...
def compareImage(img1: Image.Image, img2: Image.Image, limit=LIMIT):
...
class Window:
    ...
    def rightClick(self, pos: tuple):
        Mouse().rightClick(self.hwnd, pos)
    ...
    def frontScreenShot(self, rect: list = None):
        """
        截取指定区域的图片（前台）
        :return: PIL.Image.Image
        注：需要将屏幕缩放设置为100%，否则截取的图片不是预期的图片
        """
        windowRect = win32gui.GetWindowRect(self.hwnd)
        if rect is None:
            return ImageGrab.grab(windowRect)
        else:
            return ImageGrab.grab([rect[0]+windowRect[0],rect[1]+windowRect[1],rect[2]+windowRect[0],rect[3]+windowRect[1]])
...
if __name__ == '__main__':
    hwnds = getWindowHwnd('RenderWindow', 360, 640)
    win = Window(hwnds[0])
    win.click((301,126))
